...
function Activation() {
  ...
  const displayRazorpay = async (e) => {
    ...
    const data = await axios.post(
      "https://stormy-ridge-27884.herokuapp.com/pay/razorpay",
      {
        amount: parseInt(perid * noofid),
      }
    );
    ...
    const options = {
      ...
      handler: (response) => {
        ...
        axios
          .post(
            "https://stormy-ridge-27884.herokuapp.com/add_balance_to_activation",
            {
              token: localStorage.getItem("token"),
              amount: parseInt(perid * noofid),
            }
  ...
  return (
    <div className="active-container">
      <h2 className={`text-center ${error ? "text-danger" : "text-primary"}`}>
        {user.wallet.activation_wallet > 0
          ? "Your activation wallet balance is " + user.wallet.activation_wallet
